        ...
        <VCard>
            <VCardText class="d-flex flex-wrap py-4 gap-4">
                ...
                <div class="app-user-search-filter d-flex align-center flex-wrap gap-4">
                    <!-- 👉 Search  -->
                    <div style="inline-size: 11rem">
                        <AppTextField v-model="options.search" placeholder="Search By Question" density="compact"
                            @keyup="fetchData()" />
                    </div>
                </div>
                <div class="app-user-search-filter d-flex align-center flex-wrap gap-4">
                    <router-link to="/faq/add">
                        <VBtn prepend-icon="tabler-plus"> Add FAQ </VBtn>
                    </router-link>
                </div>
            </VCardText>
            <VDivider />
                ...
                <v-data-table class="text-no-wrap" fixed-header>
                    ...
                    <tbody>
                        <tr v-for="(item, index) in data.data" :key="index">
                            ...
                            <td class="text-center">
                                <router-link :to="'/faq/editFaq/' + item.id">
                                    <IconBtn>
...
export default {
    ...
    methods: {
        ...
        deleteData() {
            http
                .post("/faq/delete/" + this.editableId, {})
                .then((res) => {
                    if (res.data.success) {
                        this.fetchData();
